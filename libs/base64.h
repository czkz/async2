...
// appends decoded data to out
inline void from_base64(std::string_view data, std::string& out) {
    size_t oldLen = out.size();
    size_t extraLen = data.size() / 4 * 3;
    out.resize(oldLen + extraLen);
    size_t decodedLen = from_base64(data.data(), data.size(), out.data() + oldLen, extraLen);
    out.resize(oldLen + decodedLen);
}
